...
| **Productivización / Despliegue** | API FastAPI + frontal HTML/CSS/JS servido por la propia API, contenedor Docker, tests de la API, despliegue público en Render.com (plan free, ~155 MB RAM) | `api/`, `Dockerfile`, `render.yaml`, `api/test_api.py`, `deploy/INSTRUCCIONES.md` |
...
| **Visualización** | frontal con mapa Leaflet (quintiles, leyenda); dashboard de Tableau con rejilla de predicciones precalculada (`simulador_grid.csv`), riesgo por barrio/epígrafe, validación por barrio (predicho vs observado en la cohorte prospectiva) y lift por decil test vs prospectiva | `api/static/`, `src/hito13_export_tableau.py`, `src/hito21_grid_simulador.py`, `src/hito15_validacion_prospectiva.py`, `salida/tableau/` |
